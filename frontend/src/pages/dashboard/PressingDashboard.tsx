...
export function PressingDashboard() {
  ...
  const loadPressingOrders = async () => {
    try {
      setIsLoading(true);
      const response = await ordersApi.getAll({ type: 'pressing' });
      const pressingOrders = response.data;
      setOrders(pressingOrders);
      setStats({
        total: pressingOrders.length,
        enAttente: pressingOrders.filter((o) => o.status === 'en_attente').length,
        enCours: pressingOrders.filter((o) => o.status === 'en_cours').length,
        termines: pressingOrders.filter((o) => o.status === 'termine').length,
      });
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Gestion des commandes pressing"
      >
      ...
      <Card>
        <CardContent className="p-4">
          ...
          {orders.length === 0 ? (
            <EmptyState message="Aucune commande pressing" />
          ) : (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-slate-400">
                      {order.pressing?.typeVetement} × {order.pressing?.quantite}
                    </p>
                  </div>
